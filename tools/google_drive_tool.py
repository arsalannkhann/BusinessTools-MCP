...
class GoogleDriveTool(SalesTool):
    ...
    async def execute(self, action: str, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            elif action == "update_file_metadata":
            ...
            elif action == "get_file_revisions":
    ...
    async def _list_files(self, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            query_params = {
                ...
                'fields': 'nextPageToken, files(id, name, mimeType, size, createdTime, modifiedTime, owners, shared, parents, webViewLink, webContentDownloadLink)',
    ...
    async def _get_file(self, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            fields = params.get('fields', 'id, name, mimeType, size, createdTime, modifiedTime, owners, shared, parents, webViewLink, webContentDownloadLink, description, starred, trashed')
        ...
        try:
            ...
            if "content" in params:
                ...
                try:
                    ...
                    self.logger.error(f"Error reading file: {str(file_error)}")
                    return self._create_error_result(f"Error reading file: {str(file_error)}")
            ...
            result = await loop.run_in_executor(
                ...
                lambda: self.drive_service.files().create(
                    ...
                    fields='id, name, webViewLink, webContentDownloadLink, size, mimeType, createdTime'
                ).execute()
            )
            ...
            return self._create_success_result({
                ...
                'download_link': result.get('webContentDownloadLink'),
            ...
            return self._create_error_result(f"Failed to upload file: {str(e)}")
